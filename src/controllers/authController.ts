...
export const loginUser = async (ctx: Context) => {
  try {
    ...
    const userQuery = `
          ...
          FROM usuarios u
          JOIN roles r ON u.role = r.id_rol
          WHERE u.email = $1
      `;
    ...
    const token = await createJWT({
      ...
      // lastActivity: getNumericDate(0), // Marca de tiempo actual
      // exp: getNumericDate(65 * 60),
    });
    ...
    const registroResult = await db.queryObject<RegistroLogin>(
      registroLoginQuery,
      [
        user.id_usuario,
        fecha,
    ...
    // Verificar si hay un registro en `trazabilidad` con `status = FALSE`
    const trazabilidadDisponibleQuery = `
      SELECT id_trazabilidad FROM trazabilidad 
      WHERE status = FALSE 
      LIMIT 1;
  `;

    const trazabilidadResult = await db.queryObject<Trazabilidad>(
      trazabilidadDisponibleQuery,
    );

    if (trazabilidadResult.rows.length > 0) {
      // Si hay un registro disponible, reutilizarlo
      const idTrazabilidad = trazabilidadResult.rows[0].id_trazabilidad;

      const updateTrazabilidadQuery = `
        UPDATE trazabilidad 
        SET id_registro = $1, text = $2, status = TRUE 
        WHERE id_trazabilidad = $3;
    `;

      await db.queryObject(updateTrazabilidadQuery, [
        idRegistro,
        "Inicio de sesión",
        idTrazabilidad,
      ]);
    } else {
      // Si no hay un registro disponible, insertar uno nuevo
      const insertTrazabilidadQuery = `
        INSERT INTO trazabilidad (id_registro, text, status)
        VALUES ($1, $2, $3);
    `;
      await db.queryObject<Trazabilidad>(insertTrazabilidadQuery, [
        idRegistro,
        "Inicio de sesión",
        true,
      ]);
...
export const updateTrazabilidad = async (ctx: Context) => {
  try {
    ...
    const trazabilidadQuery =
      `SELECT * FROM trazabilidad WHERE id_registro = $1`;
    const trazabilidadResult = await db.queryObject<Trazabilidad>(
      trazabilidadQuery,
      [id_registro],
    );
